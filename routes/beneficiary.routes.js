// routes/beneficiary.routes.js — beneficiary endpoints
...
router.delete('/:id', requireAdmin, async (req, res) => {
  ...
  try {
    ...
    await db.execute({ sql: "DELETE FROM distributions WHERE beneficiary_id=?", args: [id] });
    await db.execute({ sql: "DELETE FROM applications WHERE beneficiary_id=?", args: [id] });
    await db.execute({ sql: "DELETE FROM beneficiaries WHERE id=?", args: [id] });
    return res.json({ message: 'Beneficiary and all associated records deleted.' });
  } catch (err) {
    console.error('DELETE /api/beneficiaries/:id error:', err);
    return res.status(500).json({ error: `Failed to delete beneficiary: ${err.message}` });
  }
});
